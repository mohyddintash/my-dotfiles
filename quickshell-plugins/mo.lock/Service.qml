...
Item {
  ...
  Process {
    id: blankProcess
    // HP Pavilion x360, AMD GPU: `omarchy-brightness-display off` dispatches
    // `hl.dsp.dpms({ action = "disable" })`, which permanently blacks this
    // panel and needs a forced reboot to recover — the BIOS never allocates
    // an LTR suspend buffer for the GPU. Omarchy 3's hypridle had an
    // OMARCHY_LOCK_ONLY escape hatch for exactly this; Quattro's stock
    // omarchy.lock has no equivalent, so this clone drops the dpms-off step.
    // Keyboard backlight off is unrelated and safe to keep.
    // See "Hardware notes" in ~/dotfiles/README.md.
